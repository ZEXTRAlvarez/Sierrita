...
async function runMigrations(database: SQLite.SQLiteDatabase): Promise<void> {
  const columns = await database.getAllAsync<{ name: string }>('PRAGMA table_info(pet_state)');
  const hasPetName = columns.some((c) => c.name === 'pet_name');
  if (!hasPetName) {
    await database.execAsync('ALTER TABLE pet_state ADD COLUMN pet_name TEXT');
  }
}
