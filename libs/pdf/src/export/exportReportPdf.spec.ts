...
describe('exportReportPdf', () => {
  ...
  it('shares the generated PDF when sharing is available', async () => {
    ...
    expect(Print.printToFileAsync).toHaveBeenCalledWith({ html: '<html></html>', base64: false });
  ...
  it('propagates a failure to generate the PDF instead of swallowing it', async () => {
    (Print.printToFileAsync as jest.Mock).mockRejectedValue(new Error('disk full'));

    await expect(exportReportPdf('<html></html>', 'Reporte')).rejects.toThrow('disk full');
  });
});
