...
export default function GameResultScreen({ summary, onPlayAgain, onBack }: Props) {
  ...
  const { scale, opacity, starsScale, starsOpacity } = useGameResultAnimations(summary.xpEarned, rewardXp);
    ...
    <View style={[styles.container, { backgroundColor: color + '22' }]}>
      <Animated.View style={[styles.card, { transform: [{ scale }], opacity }]}>
        ...
        <View style={styles.scoreBar}>
          <View style={[styles.scoreFill, { width: `${Math.round(summary.scorePercent * 100)}%` as any, backgroundColor: color }]} />
        </View>
        <Text style={styles.scoreText}>{Math.round(summary.scorePercent * 100)}%</Text>

        <View style={styles.actions}>
          <TouchableOpacity style={[styles.btn, { backgroundColor: color }]} onPress={onPlayAgain} activeOpacity={0.85}>
            <Text style={styles.btnText}>🔄 Jugar de nuevo</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.btn, styles.btnSecondary]} onPress={onBack} activeOpacity={0.85}>
