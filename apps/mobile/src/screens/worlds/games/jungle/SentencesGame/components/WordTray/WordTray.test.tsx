...
describe('WordTray', () => {
  it('shows a placeholder when no words are placed yet', () => {
    const { getByText, queryAllByTestId } = render(
      <WordTray placed={[]} status="idle" disabled={false} onRemove={jest.fn()} />,
    );
  ...
  it('renders one chip per placed word, in order', () => {
    const { getAllByTestId, getByText } = render(
      <WordTray placed={['EL', 'GATO']} status="idle" disabled={false} onRemove={jest.fn()} />,
    );
  ...
  it('calls onRemove with the tapped word and its index', () => {
    ...
    const { getAllByTestId } = render(
      <WordTray placed={['EL', 'GATO']} status="idle" disabled={false} onRemove={onRemove} />,
    );
