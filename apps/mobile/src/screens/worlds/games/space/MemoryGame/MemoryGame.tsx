...
export default function MemoryGame({ params, onRoundComplete, onGameFinish }: GameProps) {
  const pairs = (params.pairs as number) || 4;
  const flipDelay = (params.flipDelay as number) || 1200;

  const { cards, matched, finished, locked, scaleAnims, handleCardPress } = useMemoryGameState({
    pairs, flipDelay, onRoundComplete, onGameFinish,
  });
