...
describe('IdentifyMode', () => {
  it('shows the number and 4 answer options', () => {
    const { getByText, getAllByTestId } = render(
      <IdentifyMode problem={{ number: 347, hundreds: 3, tens: 4, units: 7 }} onAnswer={jest.fn()} result="idle" />,
    );
  ...
  it('reports whether the pressed option matches the correct digit', () => {
    ...
    const { getAllByTestId } = render(
      <IdentifyMode problem={{ number: 58, hundreds: 0, tens: 5, units: 8 }} onAnswer={onAnswer} result="idle" />,
    );
