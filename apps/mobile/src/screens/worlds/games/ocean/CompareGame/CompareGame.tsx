...
export default function CompareGame({
  ...
  const emojiRef = useRef(
    VISUAL_EMOJI[Math.floor(Math.random() * VISUAL_EMOJI.length)],
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.instruction}>
        {mode === 'visual' ? '¿Cuál lado tiene más?' : '¿Mayor, menor o igual?'}
      </Text>

      <Animated.View
        style={[styles.comparisonRow, { transform: [{ scale: bounceAnim }] }]}
      >
        <View style={styles.side}>
          {mode === 'visual' ? (
            ...
            <Text style={styles.expressionText}>
              {Math.floor(problem.left / 2)} +{' '}
              {problem.left - Math.floor(problem.left / 2)}
            </Text>
          ) : (
