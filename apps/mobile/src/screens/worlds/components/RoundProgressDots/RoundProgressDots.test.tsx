...
describe('RoundProgressDots', () => {
  it('renders one dot per round', () => {
    const { getAllByTestId } = render(
      <RoundProgressDots roundCount={5} rounds={[]} color="#4CAF50" />,
    );
  ...
  it('colors played dots by correctness and leaves the rest neutral', () => {
    ...
    const { getAllByTestId } = render(
      <RoundProgressDots roundCount={3} rounds={rounds} color="#4CAF50" />,
    );
    ...
    expect(dots[2].props.style).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ backgroundColor: '#DDD' }),
      ]),
    );
  });
});
